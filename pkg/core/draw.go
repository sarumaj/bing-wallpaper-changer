...
func (img *Image) DrawWatermark(watermarkFile string) error {
	...
	if watermarkBounds.Dx() < watermarkBounds.Dy() {
		...
		rotated := image.NewRGBA(image.Rect(0, 0, watermarkBounds.Bounds().Dy(), watermarkBounds.Bounds().Dx()))
		for y := watermarkBounds.Min.Y; y < watermarkBounds.Bounds().Max.Y; y++ {
			for x := watermarkBounds.Bounds().Min.X; x < watermarkBounds.Bounds().Max.X; x++ {
